...
class SalesforceXmlExtractor(LanguageExtractor):
    ...
    def extract_references(self, tree, source: bytes, file_path: str) -> list[dict]:
        refs: list[dict] = []
        self._walk_xml_refs(tree.root_node, source, refs, file_path=file_path)
        return refs
    ...
    def _walk_xml_refs(self, node, source: bytes, refs: list[dict], file_path: str):
        """Extract references from XML metadata (e.g., field references, class references)."""
        if node.type == "element":
            tag_name = self._get_element_tag(node, source)
            # Tags that reference Apex classes or other metadata
            ref_tags = {
                "apexClass", "class", "triggerType", "template",
                "customObject", "object", "relatedList",
                "lookupFilter", "referenceTo",
            }
            if tag_name and tag_name in ref_tags:
